...
namespace ClementTodd.EventGraphs.Dialogue
{
	public class DialogueManager : MonoBehaviour, IInputReceiver
	{
		...
		private DialogueNode CurrentDialogueNode
		{
			get
			{
				return EventGraphManager.Instance.CurrentEvent != null ? EventGraphManager.Instance.CurrentEvent.CurrentNode as DialogueNode : null;
			}
		}
		...
		private void OnEnable()
		{
			...
			{
				EventGraphManager.Instance.OnEventStarted += OnEventStarted;
				EventGraphManager.Instance.OnEventEnded += OnEventEnded;
			}
		}
		...
		{
			if (EventGraphManager.Instance.CurrentEvent.CurrentNode is DialogueOptionNode)
			{
				canvas.HideOptionsBox();
				EventGraphManager.Instance.CurrentEvent.ExecuteNextNode(index);
			}
			else
			{
